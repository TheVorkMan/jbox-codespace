...
# 1280x720: фиксируем 480p — CPU-энкод на 2 ядрах тянет его без лагов
...
if [ -n "$PID" ] && kill -0 "$PID" 2>/dev/null && [ "$PORT_UP" = yes ]; then
  ...
  setsid nohup /opt/selkies/selkies \
    ...
    --video-bitrate "${SELKIES_VIDEO_BITRATE:-4000,100-6000}" \
    --manual-resolution "${SELKIES_MANUAL_RESOLUTION:-true}" \
    --manual-width "${SELKIES_MANUAL_WIDTH:-1280}" \
    --manual-height "${SELKIES_MANUAL_HEIGHT:-720}" \
    --enable-resize=false \
    --audio-enabled=true \
